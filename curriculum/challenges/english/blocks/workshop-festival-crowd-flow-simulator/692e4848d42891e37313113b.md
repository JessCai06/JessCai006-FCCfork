...
This function accepts a `baseGates` array and an `options` object. 

Find the maximum queue length across all gates to determine how many ticks the simulation needs to run. Clone each gate and add runtime tracking fields: `pendingAttendees`, `throughput`, `throughputPerTick`, `peakQueue`, `reroutedOut`, and `reroutedIn`.

Use a `while` loop that continues as long as there's work to do. In each iteration, apply capacity updates to all gates, then process each gate. Increment the tick counter after each iteration.
...
Use the nullish coalescing operator (`??`) to extract `backups` and `capUpdates` from options with default values.
...
Your cloned gates should have a `throughput` property initialized to 0.
